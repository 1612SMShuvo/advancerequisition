...
// **************** ADD TO CART *******************
    ...
    public function add($item, $id, $size,$color, $keys, $values) {
        $disProd = Discount::where('product_id', '=', $id)->where('status', '=', 1)->get()->first();
        ...
        $storedItem['qty']++;
        if (!empty($disProd) && $storedItem['qty'] > $disProd->max_quantity) {
            echo ('<script>toastr.error("Reached Max Quantity !!")</script>');
            return 0;
        }
