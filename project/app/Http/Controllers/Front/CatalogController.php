...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug=null, $slug1=null, $slug2=null)
    {
        ...
        $prods = Product::when(
            $cat, function ($query, $cat) {
                                      return $query->where('category_id', $cat->id);
            }
        )
                                ->when(
                                    $subcat, function ($query, $subcat) {
                                        return $query->where('subcategory_id', $subcat->id);
                                    }
                                )
                                ->when(
                                    $childcat, function ($query, $childcat) {
                                ...
                                // search option which was working before
                                        // return $query->whereRaw('MATCH (name) AGAINST (? IN BOOLEAN MODE)', array($search));
                                // End search option which was working before
                                    }
                                )
                                ->when(
                                    $minprice, function ($query, $minprice) {
                                        return $query->where('price', '>=', $minprice);
                                    }
                                )
                                ->when(
                                    $maxprice, function ($query, $maxprice) {
                                        return $query->where('price', '<=', $maxprice);
                                    }
                                )
                                ->when(
                                    $sort, function ($query, $sort) {
                                        if ($sort=='date_desc') {
                                ...
                                ->when(
                                    empty($sort), function ($query, $sort) {
                                        return $query->orderBy('id', 'DESC');
                                    }
                                );

                                $prods = $prods->where(
                                    function ($query) use ($cat, $subcat, $childcat, $request) {
                                          $flag = 0;
        ...
        //$result = $this->loadMoreProduct($data);
