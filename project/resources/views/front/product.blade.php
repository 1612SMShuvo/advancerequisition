...
$discount_product =\DB::table('discounts')->where('product_id',$productt->id)->where('status', '=', 1)->get()->first();
          ...
          <div class="xzoom-container">
            ...
            <div class="product-price">
              <p class="title">{{ $langg->lang87 }} :</p>
                    <p class="price">
                      <span id="sizeprice">{{ !empty($discount_product) ? '*'.$curr->sign.$discount_product->discount_price*$curr->value : $productt->showPrice() }}</span>
                      <small><del>{{  !empty($discount_product) ? $productt->showPrice() : $productt->showPreviousPrice() }}</del></small>
                    </p>
                  ...
                  <div class="info-meta-3">
                    <ul class="meta-list">
                      ...
                      @else
                      <li class="addtocart">
                        <a id="add2cart" href="javascript:;" class="add-to-cart add-to-cart-btn"
                  ...
                  <div class="alert alert-warning" role="alert" style="font-size: 15px;"> * {{ $discount_product->discount_type == 1 ? '' : $curr->sign }}{{ $discount_product->discount*$curr->value }}{{ $discount_product->discount_type == 1 ? '%' : '' }} OFF with minimum purchase {{ $curr->sign.$discount_product->conditional_price*$curr->value }}<br>
                  {{ '** Maximum order quantity '.$discount_product->max_quantity }}</div>

                  {{-- DISCOUNT NOTE SECTION ENDS --}}

                  @endif

                  @if(Session::has('cart') && !empty($discount_product))

                      @foreach(Session::get('cart')->items as $product)

                        @if($product['item']['id'] == $discount_product->product_id)
                          <div class="alert alert-danger hidden">{{ 'Maximum order quantity '.$discount_product->max_quantity }}</div>
                        @endif
