			...
			<div class="row">
				...
				<div class="col-lg-8">
					...
					<form id="" action="" method="POST" class="checkoutform">
        				...
        				$discount = 0;
        				$discount_price = 0;
        				$count_discount_item =0;
        				$discount_item = [];
        				#print_r($products);
        				$discount_item_name = '';
        				$discount_item_discount = 0;
        				$discount_item_type = 0;
        				$cur = App\Models\Currency::where('is_default','=',1)->first();
        				@endphp

						@foreach($products as $product)
							@php
							$discount_product =\DB::table('discounts')->where('product_id',$product['item']['id'])->where('status', '=', 1)->get()->first();
							#print_r($discount_product);
							if(empty($discount_product)){
								$price_without_discount += $product['item']['price']*$product['qty'];
							}else{
							}
							@endphp
						@endforeach
						...
						@php 
						$pre_diff = Session::has('cart') ? Session::get('cart')->totalPrice : 0; 
						@endphp

						@foreach($products as $productts)
							@php
							$discount_product =\DB::table('discounts')->where('product_id',$productts['item']['id'])->where('status', '=', 1)->get()->first();
							if(!empty($discount_product) && $price_without_discount >= $discount_product->conditional_price){
								$diff = $price_without_discount - $discount_product->conditional_price;
								if($diff<=$pre_diff){
									$discount_item_id = $productts['item']['id'];
									$pre_diff=$diff;
								}
								$count_discount_item++;
							}
										...
										<div class="content">
											...
											<div class="ship-diff-addres-area d-none">
												...
												<div class="row">
													...
													<div class="col-lg-6">
														<input class="form-control" id="shipping_city" type="text" name="shipping_city"
										...
										<div class="content">
											...
											<div class="order-area">
												...
												@endphp
													...
													<div class="order-item">
														...
														<div class="product-content">
															...
															<div class="unit-price">
																...
																if(!empty($discount_product) && $price_without_discount >= $discount_product->conditional_price && $discount_item_id == $product['item']['id']){

																$discount_item[$product['item']['id']] = $discount_product->discount_price;
																@endphp
																	{{ App\Models\Product::convertPrice($discount_product->discount_price) }}
																@php
																}else{
																@endphp
															...
															<div class="total-price">
																...
																<p>{{ !empty($discount_product) && $price_without_discount >= $discount_product->conditional_price && $discount_item_id == $product['item']['id'] ? App\Models\Product::convertPrice($discount_product->discount_price*$product['qty']):App\Models\Product::convertPrice($product['item']['price']*$product['qty']) }}
																</p>
															</div>
														</div>
													</div>
													...
													if(!empty($discount_product)){
														if($price_without_discount >= $discount_product->conditional_price && $discount_item_id == $product['item']['id']){
															$price_with_discount += ($discount_product->discount_price*$product['qty']);
															$discount = 1;
															$discount_price += ($product['item']['price']*$product['qty'])-($discount_product->discount_price*$product['qty']);
															$discount_item_name = $discount_product->product_name;
															$discount_item_discount = $discount_product->discount;
															$discount_item_type = $discount_product->discount_type;
														}else{
															$price_with_discount += $product['item']['price']*$product['qty'];
														}
													}
														...
														<div class="nav flex-column"  role="tablist" aria-orientation="vertical">
															...
															@if($digital == 0)
																...
																@foreach($gateways as $gt)
																	...
																	<a class="nav-link payment" data-val="" data-show="yes" data-form="{{ $gt->title == 'Pay Online' ? route('gateway.payment') : route('gateway.submit') }}" data-href="{{ $gt->title == 'Pay Online' ? route('front.load.payment',['slug1' => 'online','slug2' => $gt->id]) : route('front.load.payment',['slug1' => 'bkash','slug2' => $gt->id]) }}" id="v-pills-tab{{ $gt->id }}-tab" data-toggle="pill" href="#v-pills-tab{{ $gt->id }}" role="tab" aria-controls="v-pills-tab{{ $gt->id }}" aria-selected="false">
																		<div class="icon">
																			<span class="radio"></span>
																		</div>
																		<p>
						...
						<input type="hidden" id="discount_item_name" value="{{ $discount_item_name }}">
						<input type="hidden" id="discount_item_discount" value="{{ $discount_item_type == 1 ? '' : $cur->sign }}{{ $discount_item_discount*$cur->value }}{{ $discount_item_type == 1 ? '%' : '' }}">
						<input type="hidden" id="count_discount_item" value="{{ $count_discount_item }}">

						@if($discount == 1)
							<input type="hidden" name="total" id="grandtotal" value="{{round(($price_without_discount+$price_with_discount) * $curr->value,2)}}">
							<input type="hidden" id="tgrandtotal" value="{{round(($price_without_discount+$price_with_discount) * $curr->value,2)}}">
							<input type="hidden" name="discountItems" value="{{ json_encode($discount_item) }}">
						...
						<div class="right-area">
							<div class="order-box">
								...
								<ul class="order-list">
									...
									@if($discount == 1)
										<li>
											...
											<P>
												<b class="">{{ App\Models\Product::convertPrice($discount_price) }}</b>
											</P>
										</li>
								...
								<div class="total-price">
									...
									<p>
										@if($discount == 1)
											<span id="total-cost">{{ $curr->sign }}{{round(($price_without_discount+$price_with_discount) * $curr->value,2)}}</span>
										@else
											@if(Session::has('coupon_total'))
												@if($gs->currency_format == 0)
											...
											@else
												<span id="total-cost">{{ $curr->sign }}{{round(($totalPrice) * $curr->value,2)}}</span>
											@endif
										@endif
...
@section('scripts')
	...
	<script type="text/javascript">
	    $(window).on('load', function() {
	    	var discount_item_discount = $('#discount_item_discount').val();
	        var discount_item_name = $('#discount_item_name').val();
	        var count_discount_item = $('#count_discount_item').val();
	        if (count_discount_item > 1) {
	        	$('#discount-modal').modal('show');
	        $('#discount-modal .modal-body .container').append('More than 1 discount products are added. Discount will be applicable only for the last added item which\'s conditional price is the highest and meet the requirements. In this case, you will get discount '+discount_item_discount+' on '+discount_item_name+' per unit');
	    	}
